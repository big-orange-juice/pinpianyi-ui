...
interface AppState {
  ...
  // Chat Draft (for /chat page Sender)
  chatDraft: string;
  setChatDraft: (msg: string) => void;
  ...
  addNotification: (
    n: Omit<SystemNotification, 'id' | 'timestamp' | 'isRead'>
  ) => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  chatDraft: '',
  setChatDraft: (msg) => set({ chatDraft: msg }),
  ...
  setChatHistory: (messages) =>
    set((state) => {
      const newHistory =
        typeof messages === 'function' ? messages(state.chatHistory) : messages;

      // Auto-save session
      if (newHistory.length > 1) {
        const firstUserMsg = newHistory.find((m) => m.role === 'user');
        const title = firstUserMsg
          ? firstUserMsg.content.slice(0, 18) +
            (firstUserMsg.content.length > 18 ? '...' : '')
          : '新对话';
        ...
        if (existingIndex >= 0) {
          newSessions[existingIndex] = {
            ...newSessions[existingIndex],
            title:
              newSessions[existingIndex].title === '新对话'
                ? title
    ...
    set((state) => {
      ...
      if (
        state.notifications.some(
          (n) =>
            n.title === notif.title && Date.now() - n.timestamp.getTime() < 5000
        )
      ) {
        return state;
      }
  ...
  startNewChat: () => {
    ...
    set({
      ...
      chatHistory: [
        { role: 'assistant', content: '已为您开启新会话。请问有什么可以帮您？' }
      ]
    });
  },
