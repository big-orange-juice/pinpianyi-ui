...
const AnalysisReportModal: React.FC<Props> = ({ isOpen, onClose, report }) => {
  ...
  const handleConsultAgent = (topic?: string) => {
    const query =
      topic ||
      `请基于这份《${report.title}》的"执行摘要"部分，帮我制定一个具体的周执行计划。`;
    openAgent(query);
    router.push('/chat');
  };
      ...
      <div className='bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-fade-in-up'>
        ...
        <div className='bg-slate-900 text-white p-6 flex justify-between items-start shrink-0'>
          <div>
            ...
            <p className='text-slate-400 text-sm flex items-center gap-2'>
              ...
              <span className='px-2 py-0.5 bg-blue-900/50 rounded text-xs border border-blue-800'>
                AI 深度研报
              </span>
            </p>
          </div>
        ...
        <div className='flex-1 overflow-y-auto p-8 space-y-8 bg-slate-50/50'>
            ...
            <div className='mt-4 flex items-center gap-4'>
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1 ${
                  report.marketTrend === 'Bullish'
                    ? 'bg-green-100 text-green-700'
                    : report.marketTrend === 'Bearish'
          ...
          <section>
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              {report.strategies.map((strat, idx) => (
                <div
                  key={idx}
                  className='border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow bg-white relative overflow-hidden group'>
                  <div className='absolute top-0 left-0 w-1 h-full bg-purple-500'></div>
                  <h4 className='font-bold text-slate-800 mb-2 text-sm'>
                    {strat.focus}
                  </h4>
                  <p className='text-sm text-slate-600 mb-3 h-12 line-clamp-2 group-hover:text-slate-900'>
                    {strat.action}
                  </p>
          ...
          <section className='bg-orange-50 p-6 rounded-xl border border-orange-100'>
            ...
            <p className='text-orange-700 text-sm'>{report.riskAssessment}</p>
          </section>
        </div>
        ...
        <div className='p-4 border-t border-slate-200 bg-white flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <button
              onClick={() =>
                handleConsultAgent(
                  '请针对报告中的【核心问题诊断】部分，给出更具体的落地解决方案。'
                )
              }
          ...
          <div className='flex gap-3'>
            ...
            <button
              ...
              className='px-6 py-2 bg-blue-600 text-white font-bold rounded-lg shadow hover:bg-blue-700 transition-colors flex items-center gap-2'>
